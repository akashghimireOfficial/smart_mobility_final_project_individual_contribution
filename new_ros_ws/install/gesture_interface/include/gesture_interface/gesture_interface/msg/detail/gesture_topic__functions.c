  ...
  gesture_interface__msg__GestureTopic__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    gesture_interface__msg__GestureTopic * data =
      (gesture_interface__msg__GestureTopic *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!gesture_interface__msg__GestureTopic__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          gesture_interface__msg__GestureTopic__fini(&output->data[i]);
        }
        return false;
      }
    }
